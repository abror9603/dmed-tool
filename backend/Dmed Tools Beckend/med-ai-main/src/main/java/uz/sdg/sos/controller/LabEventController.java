...
public class LabEventController {
    ...
    @PostMapping("/intake")
    @ApiOperation(value = "Receive lab result from laboratory (JWT + X-Secret-Key required)")
    public ResponseEntity<?> intake(
            @RequestHeader(value = "Authorization", required = false) String authHeader,
            @RequestHeader(value = "X-Secret-Key", required = false) String secretKey,
            @RequestBody LabEventIntakeRequest request) {

        ApiResponse<?> response = labEventService.processIntake(authHeader, secretKey, request);
